...
| `recent` | Last 14 days, planned vs actual per session, with `adherence` (`done`/`missed`) and, for completed runs, a `pace_check` (prescribed zone + window; actual pace + grade-adjusted NGP as formatted `m:ss/km`; the avg HR + its HR zone; `elevation_gain_m` (total climb); an `effort_note` when HR and pace diverge; and an on-plan/OUTSIDE-plan verdict judged on grade-adjusted pace so hills don't read as a miss) so zone adherence is judged by effort-honest pace and effort by HR × pace, not the session's name. Completed runs also carry `actual.durability` + raw `decoupling_pct`/`pace_decay_pct` (long-run aerobic drift + final-third fade) |
