...
| `src/lib/intervals-workout.ts` | normalized run segments → intervals.icu workout-builder text. Pace **bands** scale with pace (`paceBandHalfSec`: ±5 at threshold, ±30 at 5:00/km, clamped [3,45]). Pure/testable — takes `NormStep[]` so both legacy and new structure formats are handled upstream by `normalizeStructure`. |
| `src/lib/intervals-sync.ts` | `syncUpcomingRunWorkouts(days=3)` — the orchestrator: reads upcoming runs (`listUpcomingRunsForSync`), builds the workout text, creates/updates the intervals.icu event (`pushWorkoutEvent` in `intervals.ts`), stores `intervals_event_id`/`intervals_synced_at` on `plan_sessions`. Idempotent (skips sessions synced today); gated by `INTERVALS_WORKOUT_SYNC`. Called from the morning cron. |
...
| `GET/POST /api/coach/run` · `/api/coach/morning` | evening review / morning briefing (Claude → Telegram; idempotent one-per-day, `?force=1` regenerates). Morning also best-effort **pushes the next 3 days' planned runs to intervals.icu → Garmin** (`syncUpcomingRunWorkouts`, gated by `INTERVALS_WORKOUT_SYNC`) | `Bearer CRON_SECRET` **or** any authed session | cron-job.org · UI |
