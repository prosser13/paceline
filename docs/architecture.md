...
| `GET/POST /api/coach/run` · `/api/coach/morning` | evening review / morning briefing (Claude → Telegram; idempotent one-per-day, `?force=1` regenerates) | `Bearer CRON_SECRET` **or** any authed session | cron-job.org · UI |
