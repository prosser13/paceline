...
// Mint a fresh token for the current user, replacing any existing one. Returns the
// plaintext — the ONLY time it exists outside the client; the DB keeps only its hash.
export async function issueMcpToken(): Promise<string> {
  const userId = await currentUserId();
  const token = TOKEN_PREFIX + randomBytes(32).toString('base64url');
  await supabaseAdmin.from('mcp_tokens').upsert(
    { user_id: userId, token_hash: hashToken(token), created_at: new Date().toISOString(), last_used_at: null },
    { onConflict: 'user_id' },
  );
  return token;
}
...
// Resolve a bearer token to its owning user id, or null. Stamps last_used_at on a
// hit. Identity-establishing — does not (and must not) read currentUserId.
export async function resolveMcpToken(token: string | null | undefined): Promise<string | null> {
